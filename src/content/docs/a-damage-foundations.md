...
Damage multipliers of the *same* type add together. Damage multipliers of *different* types multiply with each other. This is the single most important damage principle in the game.
...
| Skill ranks (per skill tag your build uses) | Each tag is its own bucket. |
| Gem Strength | Effectively its own bucket via socketed gems. 2H weapons get double sockets → potentially 150%+ per gem at high end. |
...
On every offensive slot (gloves, amulet, rings, weapon), aim for 2–3

different

damage multiplier types: Critical Strike Damage, Vulnerable Damage, All Damage, Physical/Fire/Lightning (whatever you scale), Skill Ranks. Same-type stacking is a wasted multiplier.
:::
...
Set-charm rerolls eat

Infused Horadric Resin
...
Path: Legendary Nodes (all) → Glyph sockets → Glyph leveling → fill rare/magic nodes. Each legendary node is typically a 35–95% damage multiplier (e.g., Castle gives ~95%, Fervent gives 35% per stack with up to 3 stacks, Relentless gives 80% more).
:::

Why Your Damage Is Only Billions

### A4 — Mercenary Pick: Raheir

Universal best mercenary for damage scaling on most builds:
